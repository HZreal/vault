...
func DevTLSConfig(storageType string) (*Config, string, error) {
	...
	if err := os.WriteFile(fmt.Sprintf("%s/vault-ca.pem", dir), []byte(ca.PEM), 0644); err != nil {
		return nil, "", err
	}

	if err := os.WriteFile(fmt.Sprintf("%s/vault-cert.pem", dir), []byte(cert), 0644); err != nil {
		return nil, "", err
	}

	if err := os.WriteFile(fmt.Sprintf("%s/vault-key.pem", dir), []byte(key), 0644); err != nil {
		return nil, "", err
	}
